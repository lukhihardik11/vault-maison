    ...
    <MinimalLayout>
      ...
      <section style={{ padding: '40px 5vw 0', maxWidth: '1400px', margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '40px' }}>
          ...
          {items.length > 0 && (
            <p style={{ fontFamily: font, fontSize: '12px', color: '#9B9590' }}>{itemCount} {itemCount === 1 ? 'item' : 'items'}</p>
          )}
        </div>
      </section>
      ...
      <section style={{ padding: '0 5vw 100px', maxWidth: '1400px', margin: '0 auto' }}>
        {items.length === 0 ? (
          ...
          <div style={{ textAlign: 'center', padding: '100px 0' }}>
            ...
            <p style={{ fontFamily: font, fontSize: '13px', fontWeight: 300, color: '#9B9590', marginBottom: '32px', maxWidth: '400px', margin: '0 auto 32px' }}>
          ...
          <div className="vm-cart-layout" style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: '64px', alignItems: 'start' }}>
            ...
            <div>
              <div style={{ borderBottom: '1px solid #E8E5E0', paddingBottom: '12px', marginBottom: '24px', display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ fontFamily: font, fontSize: '11px', fontWeight: 500, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#9B9590' }}>Product</span>
                <span style={{ fontFamily: font, fontSize: '11px', fontWeight: 500, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#9B9590' }}>Total</span>
              </div>
              {items.map((item) => (
                <div key={item.product.id} style={{ display: 'flex', gap: '20px', padding: '20px 0', borderBottom: '1px solid #F5F4F0', alignItems: 'flex-start' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <p style={{ fontFamily: font, fontSize: '11px', fontWeight: 300, color: '#9B9590', marginBottom: '4px' }}>{item.product.subtitle}</p>
                    <p style={{ fontFamily: font, fontSize: '12px', fontWeight: 300, color: '#9B9590', marginBottom: '16px' }}>{item.product.material}</p>
                    {/* Quantity */}
                    <div style={{ display: 'inline-flex', alignItems: 'center', border: '1px solid #E8E5E0' }}>
                      <button onClick={() => updateQuantity(item.product.id, item.quantity - 1)} style={{ width: '32px', height: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', border: 'none', backgroundColor: 'transparent', cursor: 'pointer' }}><Minus size={12} color="#9B9590" /></button>
                      <span style={{ fontFamily: font, fontSize: '13px', fontWeight: 400, width: '32px', textAlign: 'center', color: '#1A1A1A' }}>{item.quantity}</span>
                      <button onClick={() => updateQuantity(item.product.id, item.quantity + 1)} style={{ width: '32px', height: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', border: 'none', backgroundColor: 'transparent', cursor: 'pointer' }}><Plus size={12} color="#9B9590" /></button>
                    </div>
                  </div>
            ...
            <div style={{ position: 'sticky', top: '100px' }}>
              <div style={{ padding: '32px', backgroundColor: '#F5F4F0' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px' }}>
                  <span style={{ fontFamily: font, fontSize: '13px', fontWeight: 300, color: '#9B9590' }}>Subtotal ({itemCount} items)</span>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px' }}>
                  <span style={{ fontFamily: font, fontSize: '13px', fontWeight: 300, color: '#9B9590' }}>Tax</span>
                  <span style={{ fontFamily: font, fontSize: '12px', fontWeight: 300, color: '#9B9590' }}>Calculated at checkout</span>
                </div>
                ...
                <Link href="/minimal/collections" style={{ display: 'block', textAlign: 'center', marginTop: '16px', fontFamily: font, fontSize: '12px', color: '#9B9590', textDecoration: 'underline', textUnderlineOffset: '3px' }}>
                  Continue Shopping
                </Link>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', padding: '24px 0' }}>
                {[
                  ...
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    <span style={{ fontFamily: font, fontSize: '12px', fontWeight: 300, color: '#9B9590' }}>{b.text}</span>
                  </div>
                ))}
              </div>
            </div>
