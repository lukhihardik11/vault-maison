...
export default function SalonDetail({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  return (
    <main className="min-h-screen font-lora" style={{ backgroundColor: "#FDF5E6", color: "#2B2B2B" }}>
      <nav className="fixed top-0 left-0 right-0 z-40 px-8 py-6 flex justify-between items-center backdrop-blur-sm" style={{ backgroundColor: "#FDF5E6E6" }}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          ...
          <div className="lg:sticky lg:top-28 lg:self-start">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1 }}>
              ...
              <div className="h-px mb-8" style={{ backgroundColor: "#4A5D2330" }} />
              <p className="text-2xl font-light mb-8 font-sans" style={{ color: "#4A5D23" }}>{product.priceFormatted}</p>
              ...
              <button className="w-full py-4 text-[11px] tracking-luxury-wide uppercase border transition-luxury font-sans hover:opacity-80" style={{ borderColor: "#4A5D2340" }}>
