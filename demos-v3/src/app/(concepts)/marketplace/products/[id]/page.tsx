...
export default function MarketplaceDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="min-h-screen font-space-grotesk" style={{ backgroundColor: "#1A1A1A", color: "#F2F2F2" }}>
      <nav className="fixed top-0 left-0 right-0 z-40 px-8 py-6 flex justify-between items-center backdrop-blur-sm" style={{ backgroundColor: "#1A1A1AE6" }}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          ...
          <div className="lg:sticky lg:top-28 lg:self-start">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1 }}>
              ...
              <div className="h-px mb-8" style={{ backgroundColor: "#FF3B3030" }} />
              <p className="text-2xl font-light mb-8 font-sans" style={{ color: "#FF3B30" }}>{product.priceFormatted}</p>
              ...
              <button className="w-full py-4 text-[11px] tracking-luxury-wide uppercase border transition-luxury font-sans hover:opacity-80" style={{ borderColor: "#FF3B3040" }}>
                Place Bid
              </button>
            </motion.div>
          </div>
