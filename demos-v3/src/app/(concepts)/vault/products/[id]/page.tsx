...
export default function VaultProductDetail({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          ...
          <div className="lg:sticky lg:top-28 lg:self-start">
            ...
            >
              ...
              <h1 className="text-3xl md:text-4xl tracking-[0.04em] mb-2">{product.name}</h1>
              <p className="text-[13px] opacity-50 font-light font-sans mb-8">{product.subtitle}</p>
              <div className="h-px bg-[#D4AF37]/20 mb-8" />
              <p className="text-2xl text-[#D4AF37] font-light mb-8 font-sans">{product.priceFormatted}</p>
              <p className="text-[13px] opacity-60 font-light font-sans leading-relaxed mb-10">
                {product.description}
              </p>
              ...
              <button className="w-full py-4 text-[11px] tracking-luxury-wide uppercase border border-[#D4AF37]/40 hover:bg-[#D4AF37]/10 transition-luxury font-sans">
                Request to Acquire
              </button>
            </motion.div>
          </div>
